...
#![recursion_limit = "256"]
use french_numbers::*;
use get_shell::{get_shell, Shell::*};
use std::io::{Error, Write};
use std::marker::PhantomData;

/// These constants are to avoid magic strings/values.
const LANGUAGE_LOCALES: &[&str] = &[
    "en", "es", "bg", "bn", "de", "eo", "fr", "gr", "hi", "hr", "hu", "id", "ie", "jp", "kr", "kz",
    "la", "lt", "nl", "no", "pl", "pt", "ro", "ru", "sk", "tr", "zh", "cs", "it", "uk", "ar",
];
...
trait MsgWriter<'a, T, F, Z> {
    ...
    fn write_msg(
        &mut self,
        get_actual_writer: &dyn Fn() -> Self::WriterType,
    ) -> Result<Z, std::io::Error>;
}
...
impl<'a> HelloWorldWriterCallerAndErrorHandler<'a> {
    fn new(
        language: &'a str,
    ) -> impl MsgWriterCallerAndErrorHandler<
        'a,
        ...
        (),
    > {
        HelloWorldWriterCallerAndErrorHandler { language }
    }
}
...
impl<'a, W: 'a + AnyWriter<'a, usize, ()>> MsgWriter<'a, usize, (), ()>
    for HelloWorldMsgWriter<'a, W>
{
    ...
    fn write_msg(
        &mut self,
        get_actual_writer: &dyn Fn() -> BufWriterWrapper<'a>,
    ) -> Result<(), std::io::Error> {
        ...
        if n_bytes != msg_bytes.len() {
            ...
            Err(std::io::Error::new(
                ...
                format!(
                    "Oh dear, only {} bytes were written!",
                    french_number_options(&n_bytes, &PRE_REFORM_FEMININE)
                ),
            ))
        } else {
    ...
    > for MakeAnyWriterForMakeMsgWriterForHelloWorldWriterCallerAndErrorHandler
{
    ...
    fn make_write_for_msg_writer_for_msg_writer_caller_and_error_handler(
        ...
        let buf_writer_wrapper_maker = || BufWriterWrapper::make_new_buf_writer_wrapper::<'a>();
        ...
        unsafe {
            ...
            match msg.as_str() {
                Some(msg) => {
                    ...
                    let msg_writer: HelloWorldMsgWriter<'a, BufWriterWrapper<'a>> =
                        HelloWorldMsgWriter {
                            msg,
...
impl<'a>
    MsgWriterCallerAndErrorHandler<'a, HelloWorldMsgWriter<'a, BufWriterWrapper<'a>>, usize, (), ()>
    for HelloWorldWriterCallerAndErrorHandler<'a>
{
    fn call_msg_writer_and_handle_any_errors(&self) {
        unsafe {
            ...
            let make_writer =
                MAKE_ANY_WRITER_FOR_MAKE_MSG_WRITER_FOR_HELLO_WORLD_WRITER_CALLER_AND_ERROR_HANDLER;
            let res = msg_writer.write_msg(&|| {
                (make_writer
                    .make_write_for_msg_writer_for_msg_writer_caller_and_error_handler(self)
                    .as_ref())()
            });
            match res {
                Ok(_) => {
...
fn main() {
    ...
    // SAFETY: This has been validated and independently audited for safety 🔐🚀
    unsafe {
        let hello_world_writer_caller_and_error_handler =
            HelloWorldWriterCallerAndErrorHandler::new("en");
